...
interface Notification {
  id: number;
  type: 'order_update' | 'stock_alert' | 'payment_reminder' | 'system_alert' | 'supplier_update' | 'material_request';
  title: string;
  message: string;
  is_read: boolean;
  related_id?: number;
...
export const SupplierNotifications: React.FC = () => {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      case 'material_request':
        return <TruckOutlined className="text-green-500" />;
      case 'order_update':
  ...
  const getNotificationTag = (type: string) => {
    const tagConfig = {
      supplier_update: { color: 'green', text: 'Delivery Request' },
      material_request: { color: 'green', text: 'Material Request' },
      order_update: { color: 'blue', text: 'Order Update' },
      system_alert: { color: 'orange', text: 'System Alert' },
      stock_alert: { color: 'gold', text: 'Inventory' }
    };
    ...
    <div className="p-6">
      ...
      <Card>
        {loading ? (
          ...
          <Empty 
            description="No delivery requests or notifications"
            ...
            renderItem={(notification) => (
              <List.Item
                className={`transition-all duration-200 hover:bg-gray-50 ${
                  !notification.is_read ? 'bg-green-50 border-l-4 border-l-green-500' : ''
                }`}
                actions={[
                ...
                <List.Item.Meta
                  ...
                  description={
                    <div className="mt-1">
                      ...
                      {notification.related_type === 'material_request' && notification.related_id && (
                        <div className="mt-2">
                          <Button 
                            type="link" 
                            size="small" 
